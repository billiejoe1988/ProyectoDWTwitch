...
def obtener_token():
    try:
        ...
        if response.status_code == 200:
            ...
            raise RuntimeError(f'Error al obtener el token de acceso: {response.status_code}')

    except Exception as e:
        print(f'Error en obtener_token: {str(e)}')
        raise
    ...
    try:
        ...
        if access_token:
            ...
            if response.status_code == 200:
                ...
                # Verificar valores nulos y eliminar valores nulos
                ...
                # Confirmar cambios, cerrar cursor y conexión
                conn.commit()
                cur.close()
                conn.close()

            else:
                raise RuntimeError(f'Error al realizar la solicitud: {response.status_code}')

        else:
            raise RuntimeError('No se pudo obtener el token de acceso.')

    except Exception as e:
        print(f'Error en extraer_datos: {str(e)}')
        raise
